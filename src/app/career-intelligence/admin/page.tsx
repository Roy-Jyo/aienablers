...
export default function CareerIntelligenceAdminPage() {
  ...
  async function loadFeeds() {
    setStatus("loading");
    setMessage("");
    sessionStorage.setItem("career-admin-key", adminKey);
    const response = await fetch("/api/career-intelligence/employers", { headers: { "x-admin-key": adminKey }, cache: "no-store" });
  ...
  async function setEnabled(feed: Feed, enabled: boolean) {
    ...
    const data = await response.json();
    if (!response.ok) return setMessage(data.error ?? "Could not update employer.");
    await loadFeeds();
  }
  ...
  async function removeEmployer(feed: Feed) {
    ...
    const data = await response.json();
    if (!response.ok) return setMessage(data.error ?? "Could not delete employer.");
    await loadFeeds();
  }
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-slate-100">
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          <div className="mt-2 flex flex-col gap-3 sm:flex-row"><div className="flex flex-1 items-center rounded-xl border border-slate-700 bg-slate-950 px-3"><LockKeyhole size={18} className="text-slate-500"/><input type="password" value={adminKey} onChange={(e)=>setAdminKey(e.target.value)} className="w-full bg-transparent px-3 py-3 text-white outline-none" placeholder="Enter CAREER_ADMIN_KEY"/></div><button onClick={loadFeeds} disabled={!adminKey||status==="loading"} className="rounded-xl bg-indigo-600 px-6 py-3 font-semibold hover:bg-indigo-500 disabled:opacity-50">{status==="loading"?<span className="flex items-center gap-2"><Loader2 size={18} className="animate-spin"/> Loading</span>:"Open console"}</button></div>
