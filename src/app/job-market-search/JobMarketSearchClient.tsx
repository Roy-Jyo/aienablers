...
import { Bot, ExternalLink, Loader2, MapPin, Search, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-gradient-to-b from-indigo-50 via-white to-slate-50">
      <section className="mx-auto max-w-7xl px-6 py-14 sm:py-20">
        <div className="mx-auto max-w-4xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-white px-4 py-2 text-sm font-medium text-indigo-700 shadow-sm">
        ...
        <form onSubmit={searchJobs} className="mx-auto mt-10 max-w-5xl rounded-3xl border border-indigo-100 bg-white p-5 shadow-2xl shadow-indigo-100/70 sm:p-7">
          <label className="block">
            <span className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-700">
              <Sparkles size={17} className="text-indigo-600" /> Describe your ideal role
            </span>
            <div className="rounded-2xl border border-slate-300 bg-slate-50/60 p-3 focus-within:border-indigo-500 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-100">
              <textarea
                value={query}
                ...
                className="w-full resize-none bg-transparent px-2 py-2 text-lg leading-7 outline-none placeholder:text-slate-400"
              />
            </div>
          </label>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-[1fr_auto]">
            <label className="block">
              ...
              <div className="flex items-center rounded-xl border border-slate-300 px-3 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
            ...
            <button type="submit" disabled={status === "loading"} className="self-end rounded-xl bg-indigo-600 px-7 py-3 font-semibold text-white shadow hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
              {status === "loading" ? (
        ...
        {status === "idle" && (
          <div className="mx-auto mt-8 max-w-5xl">
            <p className="text-center text-sm font-medium text-slate-500">Try an example</p>
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              {examples.map((example) => (
                <button key={example} type="button" onClick={() => setQuery(example)} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600 shadow-sm hover:border-indigo-300 hover:text-indigo-700">
                  {example}
                </button>
              ))}
            </div>
        ...
        {status === "error" && (
          <div className="mx-auto mt-8 max-w-5xl rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">{message}</div>
        )}
        ...
        {status === "done" && (
          <section className="mx-auto mt-12 max-w-5xl" aria-live="polite">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
              ...
              {total > results.length && <p className="text-sm text-slate-500">{total.toLocaleString()} total market matches</p>}
            </div>
            ...
            <div className="grid gap-5">
              {results.map((job) => {
                ...
                return (
                  <article key={job.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <a href={job.url} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-indigo-200 px-4 py-2 font-medium text-indigo-700 hover:bg-indigo-50">
                        View Job <ExternalLink size={16} />
                      </a>
                    </div>
                    <p className="mt-4 line-clamp-3 leading-7 text-slate-600">{stripHtml(job.description)}</p>
                    <p className="mt-4 text-xs text-slate-400">Market source: {job.source}</p>
                  </article>
                );
              })}
            </div>
